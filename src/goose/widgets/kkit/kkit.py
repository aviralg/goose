...
class  KineticsWidget(QtGui.QWidget):
    ...
    def setupDisplay(self,info,graphicalObj,objClass):
        ...
        '''
        if isinstance(self,widgets.kkit.kkit.KineticWidget):
            ...
            editorItem = self.editormooseId_GObj[self.moose.element(info).parent]
                ...
                if k.path == mesh:
                    for rectChilditem in v.childItems():
                        if isinstance(rectChilditem, KineticsDisplayItem):
                            if isinstance(self.moose.element(rectChilditem.mobj.path),self.moose.PoolBase):
                                t = self.moose.element(rectChilditem.mobj.path)
                                self.moose.element(t).children
                                for items in self.moose.element(t).children:
